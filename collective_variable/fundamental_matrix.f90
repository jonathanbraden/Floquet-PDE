...
! First, get the modulus of the eigenvalues (we don't really care about the phases for the moment
  ...
  subroutine get_eigenfunction(eigenfuncs, floquet_exp, nevalues)
    real(dl), dimension(1:nlat, 1:4*nevalues), intent(out) :: eigenfuncs
    real(dl) :: floquet_exp(nevalues)
    integer, intent(in) :: nevalues
    ...
    write(99,*) "# x f(x), pi(x) (x2), F_{coeffs}, pi_{basis} (x2), floquet"

    evalreal = evalreal**2 + evalimag**2
    do i=1,nevalues
       maxexp = maxval(evalreal)
       floquet_exp(i) = 0.5*log(maxexp)
       maxind = maxloc(evalreal)
       evalreal(maxind(1)) = 0.

! Now extract the Fourier coefficients
       eigenfuncs(1:nlat,4*(i-1)+1) = leftvectors(1:nlat, maxind(1))
       vectmp(:,1) = matmul(eigenfuncs(:,1),basis)
       eigenfuncs(1:nlat,4*(i-1)+2) = leftvectors(nlat+1:2*nlat,maxind(1))
       vectmp(:,2) = matmul(eigenfuncs(:,2),basis)
       eigenfuncs(1:nlat,4*(i-1)+3) = rightvectors(1:nlat, maxind(1))
       vectmp(:,3) = matmul(eigenfuncs(:,3),basis)
       eigenfuncs(1:nlat,4*(i-1)+4) = rightvectors(nlat+1:2*nlat,maxind(1))
       vectmp(:,4) = matmul(eigenfuncs(:,4),basis)
       do j=1,nlat
          write(99,'(10(ES14.7,2x))') j*dx, eigenfuncs(j,4*(i-1)+1), eigenfuncs(j,4*(i-1)+2), &
               eigenfuncs(j,4*(i-1)+3), eigenfuncs(j,4*(i-1)+4), &
               vectmp(j,1), vectmp(j,2), &
               vectmp(j,3), vectmp(j,4), floquet_exp(i)
       enddo
       write(99,*)""
    enddo

    print*, "# amp, k2, lyapanov exponent, imaginary piece"
    print*, output_param(), k2, floquet_exp, evalimag(maxind(1))
